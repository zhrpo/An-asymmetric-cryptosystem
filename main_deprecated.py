#DEPRECATED
import random, sys, os, rabinMiller_deprecated, euclidAlg_deprecated, math, keyGen
...
def genKey(keySize):
    p = rabinMiller_deprecated.generateLargePrime(keySize)
    q = rabinMiller_deprecated.generateLargePrime(keySize)
    n = p * q
    while True:
        e = random.randrange(2 ** (keySize - 1), 2 ** (keySize))
        if euclidAlg_deprecated.gcd(e, (p - 1) * (q - 1)) == 1:
            break

    d = euclidAlg_deprecated.modInverse(e, (p - 1) * (q - 1))
    publicKey = (n, e)
    privateKey = (n, d)
